...
serve(async (req) => {
  ...
  try {
    ...
    // Find user by email - fix: add proper query parameter
    ...
    console.log(`Cercando utente con email: ${email} tra ${users?.users.length || 0} utenti`);
    
    // Check if email exists in lowercase or with different case
    const userAccount = users?.users.find(u => 
      u.email && u.email.toLowerCase() === email.toLowerCase()
    );

    if (!userAccount) {
      // If user not found in auth, create a new user
      console.log(`Utente con email ${email} non trovato in auth. Creazione nuovo utente...`);
      ...
      return new Response(
        JSON.stringify({ message: "UTENTE CREATO E PASSWORD IMPOSTATA CON SUCCESSO" }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
